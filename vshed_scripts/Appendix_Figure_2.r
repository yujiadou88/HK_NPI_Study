...
# body temperature
symp$bodytemp[symp$bodytemp==0] <- NA
symp <- merge(symp[1:4],pcr[c(1,2,11)],all.y=T)
symp$day <- symp$day+symp$v1_day
pcr3 <- merge(pcr2[-5],symp[1:4],by=c("hhID","member","day"))
...
# Day -1 : Day 0
for (i in 1:7){
    plot(pcr3$bodytemp[pcr3$day == (i-2) & pcr3$QVres == 1 & pcr3$adult == 1], pcr3$logpcr[pcr3$day == (i-2) & pcr3$QVres == 1 & pcr3$adult == 1]
         ,type ='p', axes=FALSE, xlim=c(34,41), ylim=c(2,10), bty='n', cex=1, xlab="", ylab="")
    points(pcr3$bodytemp[pcr3$day == (i-2) & pcr3$QVres == 1 & pcr3$adult == 0], pcr3$logpcr[pcr3$day == (i-2) & pcr3$QVres == 1 & pcr3$adult == 0]
           , cex=0.8, pch=3)
    axis(1, pos=2, lwd=1, font=1, at=c(34:41), label=c(34:41), cex.axis=1,mgp=c(1.5,0.6,0))
    axis(2, pos=34, lwd=1, font=1, at=c(2:10),
    ...
    if(length(na.exclude(pcr3$bodytemp[pcr3$day == (i-2) & pcr3$QVres == 1]))>2){
      lm <- lm(pcr3$logpcr[pcr3$day == (i-2) & pcr3$QVres == 1] ~ pcr3$bodytemp[pcr3$day == (i-2) & pcr3$QVres == 1])
      x1 <- range(pcr3$bodytemp[pcr3$day == (i-2) & pcr3$QVres == 1],na.rm=T)[1] - 0.1
      x2 <- range(pcr3$bodytemp[pcr3$day == (i-2) & pcr3$QVres == 1],na.rm=T)[2] + 0.1
      y1 <- summary(lm)[[4]][1] + x1 * summary(lm)[[4]][2]
      y2 <- summary(lm)[[4]][1] + x2 * summary(lm)[[4]][2]
      lines(c(x1,x2),c(y1,y2), lwd=1.15)
      mtext(paste("slope =",round(summary(lm)[[4]][2],2)), side=1, line=-3, cex=0.6, at=40)
      mtext(paste("p-value =",round(summary(lm)[[4]][8],2)), side=1, line=-2.2, cex=0.6, at=40)
    }
}
...
mtext(expression(bold(paste("Temp (",degree,"C)", sep=""))), SOUTH <- 1, cex=0.7, line=-0.3, at=0.13, font=2, outer=TRUE)
mtext(expression(bold(paste("Temp (",degree,"C)", sep=""))), SOUTH <- 1, cex=0.7, line=-0.3, at=0.38, font=2, outer=TRUE)
mtext(expression(bold(paste("Temp (",degree,"C)", sep=""))), SOUTH <- 1, cex=0.7, line=-0.3, at=0.63, font=2, outer=TRUE)
mtext(expression(bold(paste("Temp (",degree,"C)", sep=""))), SOUTH <- 1, cex=0.7, line=-23, at=0.88, font=2, outer=TRUE)
