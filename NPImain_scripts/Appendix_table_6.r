#
# Script to reproduce information in Appendix Table 6 from:
#
...
appt6 <- matrix(rep(NA,221),ncol=13,byrow=FALSE)
...
appt6[c(1,4,7,9,11,14,16),3*i-1] <- 1.00
appt6[c(2,3,5,6,8,10,12,13,15,17),(i-1)*3+2:4] <- cbind(results$OR[-1],results$lower.CI[-1],results$upper.CI[-1])
}
...
appt6[,1] <- c(dim(new[new$arm=="control",])[1],dim(new[new$arm=="hand",])[1],dim(new[new$arm=="handmask",])[1],
...
rownames(appt6) <- c("control","hand","handmask","adult","child6-15","child<=5","female","male","not vac","vaccine",
              "adult index","child6-15 index","child<=5 index","female index","male index","no antiviral","antiviral")
colnames(appt6) <- c("n","lab or cdef1 - OR","CI-low","CI-up","lab and cdef1 - OR","CI-low","CI-up",
