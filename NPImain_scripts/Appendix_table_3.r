#
# Script to reproduce information in Appendix Table 3 from:
#
...
appt3 <- matrix(rep(NA,60),ncol=10,byrow=FALSE)
...
hculture <- merge(hculture,cdat[,c(2,13)],by="hhID",all.x=TRUE)
...
hc_contact <- hculture.cc[hculture.cc$QVres==1,c(1,2,10,14:16)]   # Flu A only
...
set.seed(12345)
cresult <- cbCI(c2nd_all,1000)
hresult <- cbCI(h2nd_all,1000)
mresult <- cbCI(m2nd_all,1000)
...
appt3[1,] <- c(cresult,hresult,mresult,p)
...
set.seed(12345)
cresult <- cbCI(c2nd_s,1000)
hresult <- cbCI(h2nd_s,1000)
mresult <- cbCI(m2nd_s,1000)
...
appt3[4,] <- c(cresult,hresult,mresult,p)
...
hclinic$QVres <- hculture$QVres
clicon <- hclinic[hclinic$member>0 & hclinic$analyzed==1 & hclinic$QVres==1 ,]   # Flu A only
clicon$hhsize <- hc_contact$hhsize

##################################################### Full data ############################################################

# SAR & cluster bootstrap CIs
c2nd <- clicon[clicon$arm=="control",]
h2nd <- clicon[clicon$arm=="hand",]
m2nd <- clicon[clicon$arm=="handmask",]
set.seed(123456)
cresult <- cbCI2(c2nd,1000)
hresult <- cbCI2(h2nd,1000)
mresult <- cbCI2(m2nd,1000)
...
appt3[1+k,] <- c(cresult,hresult,mresult,p)
...
appt3[4+k,] <- c(cresult,hresult,mresult,p)

}

rownames(appt3) <- c("Any-lab","Any-cd1","Any-cd2","<36-lab","<36-cd1","<36-cd2")
colnames(appt3) <- c("Control","CI-low","CI-up","Hany hygiene","CI-low","CI-up","Mask+HH","CI-low","CI-up","p-value")
appt3
