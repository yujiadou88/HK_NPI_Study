#
# Script to reproduce information in Appendix Table 4 from:
#
...
hc_contact <- hculture.cc[hculture.cc$QVres==2,c(1,2,10,14:16)]   # Flu B only
...
appt4[1,] <- c(cresult,hresult,mresult,p)
...
appt4[4,] <- c(cresult,hresult,mresult,p)
...
clicon <- hclinic[hclinic$member>0 & hclinic$analyzed==1 & hclinic$QVres==2 ,]   # Flu B only
...
appt4[1+k,] <- c(cresult,hresult,mresult,p)
...
appt4[4+k,] <- c(cresult,hresult,mresult,p)

}

rownames(appt4) <- c("Any-lab","Any-cd1","Any-cd2","<36-lab","<36-cd1","<36-cd2")
colnames(appt4) <- c("Control","CI-low","CI-up","Hany hygiene","CI-low","CI-up","Mask+HH","CI-low","CI-up","p-value")
appt4
