#
# Script to reproduce information in Appendix Table 9 from:
#
...
# Analyzed households: 191 households (<36h)

appt9 <- matrix(rep(NA,190),ncol=10,byrow=FALSE)
...
model1 <- model2 <- model3 <- model4  <- model5 <- model6 <-  list(NA)

for(i in 1:10){
      model1[[i]] <- gee(labsedcase ~ factor(intervention)+factor(agegp)+sex+vaccine+factor(indexagegp)+indexsex+indexav+co_index  ,
                         data=data.nomiss[[i]][data.nomiss[[i]]$delay=="d1",], id=factor(data.nomiss[[i]]$hhID[data.nomiss[[i]]$delay=="d1"]), corstr = "exchangeable", family="binomial")
...
appt9[c(1,4,7,9,11,14,16,18),c(2,5,8)] <- 1
cm1 <- combine.mi(model1, n.impute=10)
cm2 <- combine.mi(model2, n.impute=10)
cm3 <- combine.mi(model3, n.impute=10)
appt9[c(2,3,5,6,8,10,12,13,15,17,19),2:4] <- cbind(cm1$OR[-1],cm1$lowerCI[-1],cm1$upperCI[-1])
appt9[c(2,3,5,6,8,10,12,13,15,17,19),5:7] <- cbind(cm2$OR[-1],cm2$lowerCI[-1],cm2$upperCI[-1])
appt9[c(2,3,5,6,8,10,12,13,15,17,19),8:10] <- cbind(cm3$OR[-1],cm3$lowerCI[-1],cm3$upperCI[-1])

#### n 

new <- data.nomiss[[1]][data.nomiss[[1]]$delay=="d1",]
dim(new)
length(unique(new$hhID))

appt9[,1] <- c(dim(new[new$intervention==1,])[1],dim(new[new$intervention==3,])[1],dim(new[new$intervention==4,])[1],
...
rownames(appt9) <- c("control","hand","handmask","adult","child6-15","child<=5","female","male","not vac","vaccine",
              "adult index","child6-15 index","child<=5 index","female index","male index","no antiviral","antiviral","no co-index","co_index")
colnames(appt9) <- c("n","lab-OR","CI-low","CI-up","cd1-OR","CI-low","CI-up","cd2-OR","CI-low","CI-up")
